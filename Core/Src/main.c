...
//
//*****************************************************************************
static void CallApplication(uint_fast32_t ui32StartAddr)
{
typedef void (*jump_app)(void);

	volatile uint32_t *_vectable = (__IO uint32_t*)0x08010000;  // point _vectable to the start of the application at 0x08005000
	...
	SCB->VTOR = (volatile uint32_t)_vectable;   // point VTOR to the start of the application's vector table
...
int main(void)
{
  ...
  if (CheckIntelHexFile(EXT_FLASH_APPL_STARTING_ADDRESS, &StartAddress, &EndAddress)) {
   		if ((StartAddress == APP_START_ADDRESS) && (EndAddress < 0x040000)) {
   		    ...
   		    for(Address = StartAddress; Address < EndAddress; Address += 1024) {
  // 		        ROM_FlashErase(Address);
   		    }
   		    Address = 0;
   			f = FileOpen('r', EXT_FLASH_APPL_STARTING_ADDRESS);
   			while(f && ReadIntelHexLine(f, &r) && (EndOfFileMarker == false)){
   				switch(r.Recordtype){
 //  				case 0: ROM_FlashProgram((uint32_t *)r.Data, Address + r.Address, (r.ByteCount + 3) & ~3);	break;
   				case 1: EndOfFileMarker = true;																break;
   				case 4: Address = 0x00010000 * ((r.Data[0] << 8) + r.Data[1]);								break;
   				}
   			}
   			FileClose(f);
   		}
   		FileDelete(EXT_FLASH_APPL_STARTING_ADDRESS);
   	}
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(INP_LATCH_GPIO_Port, INP_LATCH_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, FLASH_NSS_Pin|OUTP_OE_Pin|OUTP_LATCH_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, TP5_Pin|INP_NSS_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(TP6_GPIO_Port, TP6_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(EEPROM_NSS_GPIO_Port, EEPROM_NSS_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : INP_LATCH_Pin FLASH_NSS_Pin OUTP_OE_Pin OUTP_LATCH_Pin */
  GPIO_InitStruct.Pin = INP_LATCH_Pin|FLASH_NSS_Pin|OUTP_OE_Pin|OUTP_LATCH_Pin;
  ...
  /*Configure GPIO pins : TP5_Pin INP_NSS_Pin */
  GPIO_InitStruct.Pin = TP5_Pin|INP_NSS_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  /*Configure GPIO pin : TP6_Pin */
  GPIO_InitStruct.Pin = TP6_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(TP6_GPIO_Port, &GPIO_InitStruct);
